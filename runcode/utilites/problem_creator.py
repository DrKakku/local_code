...
def list_templates() -> List[str]:
    ...
    try:
        templates = [p.name for p in TEMPLATES_DIR.glob("*.py") if p.name != "__init__.py"]
        if not templates:
...
def gather_test_cases() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        while Prompt.ask("Add test case? (y/n)", choices=["y", "n"], default="y") == "y":
            inp = escape(Prompt.ask("Enter input tuple, e.g. ([1,2], 3)"))  # Escape input
            out = escape(Prompt.ask("Enter expected output (leave blank for ref)"))  # Escape output
...
def main() -> None:
    ...
    try:
        ...
        description = get_multiline_input("Enter detailed description (end with blank line):")
