...
def log_submission(problem: str, case_idx: int, status: str, duration: float, expected: str, actual: str, solution: str):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
...
def fetch_history(problem: str = None, entry_id: int = None):
    ...
    if entry_id is not None:
        ...
        c.execute('SELECT * FROM submissions WHERE problem = ? ORDER BY timestamp DESC', (problem,))
    else:
